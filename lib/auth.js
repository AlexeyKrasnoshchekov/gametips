...
export function createUser({ email, name, provider, password }) {
  ...
  const user = {
    ...
    name: (name || normalizedEmail.split('@')[0]).trim().slice(0, 60),
    provider,
    createdAt: new Date().toISOString(),
  };
  if (password) {
    user.salt = randomBytes(16).toString('hex');
    user.hash = scryptSync(String(password), user.salt, 64).toString('hex');
  }
  users.push(user);
  writeUsers(users);
  return user;
...
// Simplified Google sign-up: if the email already has an account (registered
// via email earlier), just sign into it; otherwise create one — no password.
export function attachGoogleProfile({ email, name }) {
  const users = readUsers();
  const normalizedEmail = String(email).trim().toLowerCase();
  ...
  if (user) {
    ...
    if (name && (!user.name || user.name === user.email)) user.name = String(name).slice(0, 60);
    writeUsers(users);
    return user;
  }
  return createUser({ email: normalizedEmail, name, provider: 'google' });
}
...
export function createSessionToken(user) {
  const payload = Buffer.from(
    JSON.stringify({ uid: user.id, email: user.email, name: user.name, exp: Date.now() + SESSION_TTL_MS }),
  ).toString('base64url');
