...
// посчитал пики (crowd consensus + AI) и сохранил. Без ?date= — самая свежая
// дата в базе. Ответ — плоский массив пиков вида
...
//     aiNote, homeTeam, awayTeam, league, date, matchId, ... }
export async function fetchSelectedBestPicks(date) {
  const url = date
    ? `${API_BASE}/selectBestPicks/saved?date=${encodeURIComponent(date)}`
    : `${API_BASE}/selectBestPicks/saved`;
  const MAX_ATTEMPTS = 3;
  let lastError = null;
