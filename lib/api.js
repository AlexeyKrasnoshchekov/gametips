...
export const API_BASE = (
  process.env.NEXT_PUBLIC_API_BASE_URL || 'https://api.gametips.bet/prod'
).replace(/\/+$/, '');
...
// GET /todayPicks/get — сегодняшние Best Picks (загружаются через gametips-dash).
...
export async function fetchTodayPicks() {
  const url = `${API_BASE}/todayPicks/get`;
  const MAX_ATTEMPTS = 3;
  let lastError = null;
