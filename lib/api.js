...
//   Under 3.5 -> probWeightUnder35  (no per-match source count on the API)
...
export function getFilterCount(match, filterType) {
  ...
  switch (filterType) {
    ...
    case 'Under 3.5':
      // У рынка Under 3.5 нет агрегата источников на API — сортируем по весу
      // модели, чтобы матчи с данными Under 3.5 оказались выше остальных.
      return num(match.probWeightUnder35);
    case 'BTTS':
      return num(
...
// GET /selectBestPicks/saved?date=dd.MM.yyyy — уже сохранённые пики серверного
// расчёта (коллекция SelectedBestPicks): сервер взял матчи из mc за дату,
// посчитал пики (crowd consensus + AI + markTopPicks) и сохранил. Без ?date= —
// самая свежая дата в базе. Ответ — плоский массив пиков вида
//   { pickType, confidence, base, sourceCount, threshold, isCSEnhanced,
//     csAvgGoals, isAIConfirmed, aiConfirmedConfidence, aiConfirmedBase,
//     aiNote, isTopPick, topPickRank, homeTeam, awayTeam, league, date,
//     matchId, ... }
// fetchJsonArray — общий загрузчик: retries с бэкоффом (API за Cloudflare
// иногда роняет первую попытку), 404 не ретраится — маршрута на сервере нет.
async function fetchJsonArray(url, { attempts = 3, timeoutMs = 30000 } = {}) {
  let lastError = null;
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      const res = await fetch(url, {
        headers: { 'Accept': 'application/json' },
        // Never cache — picks are refreshed daily via the server-side recalculation.
        cache: 'no-store',
      ...
      lastError = err;
      // 404 — маршрута нет на этом бэкенде, повтором не помочь.
...
export async function fetchSelectedBestPicks(date) {
  const query = date ? `?date=${encodeURIComponent(date)}` : '';

  // 1) Основной путь — уже сохранённые пики (быстро, без записи в базу).
  try {
    return await fetchJsonArray(`${API_BASE}/selectBestPicks/saved${query}`);
  } catch (err) {
    // 2) Fallback на случай старого бэкенда без маршрута /saved (404):
    //    compute-эндпоинт GET /selectBestPicks?date= есть во всех версиях
    //    сервера с роутером selectBestPicks — он пересчитает пики, сохранит
    //    их в SelectedBestPicks и вернёт. На актуальном бэкенде этот путь
    //    не выполняется никогда (там /saved отвечает 200).
    if (err && err.status === 404) {
      return fetchJsonArray(`${API_BASE}/selectBestPicks/${query}`, {
        attempts: 2,
        // Пересчёт (чтение mc + расчёт + запись) дольше чтения /saved.
        timeoutMs: 60000,
      });
    }
    throw err;
...
// GET /todayBestPicks/get?date=dd.MM.yyyy — Best Picks за конкретную дату.
...
// — коллекция TodayBestPicks заполняется загрузкой JSON-файла в дашборде
// (см. mongo_schema/TodayBestPicks на бэкенде).
export async function fetchTodayBestPicks(date) {
  const url = date
    ? `${API_BASE}/todayBestPicks/get?date=${encodeURIComponent(date)}`
    : `${API_BASE}/todayBestPicks/get`;
  const MAX_ATTEMPTS = 3;
  let lastError = null;
